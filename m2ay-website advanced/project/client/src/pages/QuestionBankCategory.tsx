...
export default function QuestionBankCategory() {
  ...
  const { data: categories } = trpc.public.questionBank.categories.useQuery();
  const { data: questions, isLoading } = trpc.public.questionBank.questionsByCategory.useQuery({ categoryId });
  ...
  useSEO({
    ...
    description: isAr
      ? `${questions?.length ?? 0} سؤال مقابلة حقيقي في هذه الفئة، بإجابات نموذجية وأخطاء شائعة.`
      : `${questions?.length ?? 0} real interview questions in this category, with model answers and common mistakes.`,
        ...
        {category && (
          <>
            ...
            <p className="text-muted-foreground mb-8">
              {isAr ? `${questions?.length ?? 0} سؤال في هذه الفئة` : `${questions?.length ?? 0} questions in this category`}
            </p>
          </>
        )}
