...
export default function QuestionBank() {
  ...
  useSEO({
    ...
    description: isAr
      ? "أكثر من 160 سؤال حقيقي شائع في مقابلات التمويل والمحاسبة، بإجابات نموذجية وأخطاء شائعة."
      : "160+ real finance & accounting interview questions with model answers and common mistakes.",
    path: "/question-bank",
  });

  const { data: categories, isLoading } = trpc.public.questionBank.categories.useQuery();
    ...
    <div dir={isAr ? "rtl" : "ltr"} className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <p className="text-muted-foreground mb-8">
          {isAr
            ? `${totalQuestions} سؤال حقيقي شائع في مقابلات التمويل والمحاسبة، موزعة على ${categories?.length ?? 0} فئة`
            : `${totalQuestions} real, common finance & accounting interview questions across ${categories?.length ?? 0} categories`}
        </p>
