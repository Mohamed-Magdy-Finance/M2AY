...
export default function ChapterDetail() {
  ...
  const { data: chapter, isLoading } = trpc.public.chapters.byId.useQuery({ id: chapterId });
  ...
  useJsonLd(
    chapter
      ? {
          ...
          "@type": "Article",
          headline: isAr ? chapter.arabicTitle : chapter.englishTitle,
  ...
  if (isLoading) {
    return <p className="text-center py-32 text-muted-foreground">{isAr ? "جاري التحميل..." : "Loading..."}</p>;
  }
  ...
  return (
    <div dir={isAr ? "rtl" : "ltr"} className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <Badge variant="outline" className="mb-3">{chapter.section}</Badge>
        ...
        <article className="prose dark:prose-invert max-w-none whitespace-pre-wrap leading-relaxed mb-12">
          {isAr ? (
            chapter.arabicContent
          ) : chapter.englishContent ? (
            chapter.englishContent
          ) : (
            <>
        ...
        {/* Related templates — Content Relationship Engine */}
        {chapter.relatedTemplates && chapter.relatedTemplates.length > 0 && (
          <div className="mb-10">
            <h2 className="font-bold text-lg mb-4">{isAr ? "قوالب مرتبطة بهذا الفصل" : "Templates related to this chapter"}</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {chapter.relatedTemplates.map(t => (
        ...
        {/* Related question bank categories */}
        {chapter.relatedQuestionCategories && chapter.relatedQuestionCategories.length > 0 && (
          <div className="mb-10">
            <h2 className="font-bold text-lg mb-4">{isAr ? "أسئلة مقابلات مرتبطة" : "Related interview questions"}</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {chapter.relatedQuestionCategories.map(cat => (
        ...
        {(chapter.previousChapter || chapter.nextChapter) && (
          <div className="grid sm:grid-cols-2 gap-4 pt-6 border-t border-border">
            {chapter.previousChapter ? (
              <Link href={lp(`/chapters/${chapter.previousChapter.id}`)}>
                <Card className="p-4 hover:shadow-lg transition-shadow cursor-pointer h-full">
                  ...
                  <p className="font-semibold text-sm">{isAr ? chapter.previousChapter.arabicTitle : chapter.previousChapter.englishTitle}</p>
                </Card>
              </Link>
            ) : <div />}
            {chapter.nextChapter && (
              <Link href={lp(`/chapters/${chapter.nextChapter.id}`)}>
                <Card className="p-4 hover:shadow-lg transition-shadow cursor-pointer h-full text-end">
                  ...
                  <p className="font-semibold text-sm">{isAr ? chapter.nextChapter.arabicTitle : chapter.nextChapter.englishTitle}</p>
                </Card>
              </Link>
            )}
          </div>
