...
export default function Home() {
  ...
  const { data: chapters } = trpc.public.chapters.list.useQuery();
  const { data: templates } = trpc.public.templates.list.useQuery();
    ...
    <div dir={isAr ? "rtl" : "ltr"} className="min-h-screen bg-background text-foreground">
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold mb-4 leading-tight">
              {profile?.fullName || "محمد مجدي"}
            </h1>
            <p className="text-lg opacity-90 mb-2">
              {profile?.title || "FP&A Analyst"}
            </p>
          ...
          >
            <div className="relative">
              ...
              <img
                ...
                alt={profile?.fullName || "Mohamed Magdy"}
                fetchPriority="high"
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {(certifications.length > 0 ? certifications : ["CFI", "PFA", "Oracle Financials", "Xero"]).map((cert, i) => (
            ...
            {featuredChapters.map(ch => (
              <Link key={ch.id} href={lp(`/chapters/${ch.id}`)}>
                <Card className="p-5 h-full hover:shadow-lg transition-shadow cursor-pointer">
                  ...
                  <p className="text-sm opacity-70">{isAr ? ch.englishTitle : ch.arabicTitle}</p>
                </Card>
              </Link>
            ))}
          </div>
          ...
          {featuredTemplates.map(t => (
            <Link key={t.id} href={lp(`/templates/${t.id}`)}>
              <Card className="p-5 h-full hover:shadow-lg transition-shadow cursor-pointer">
                ...
                <h3 className="font-bold text-sm mb-2">{isAr ? t.arabicName : t.englishName}</h3>
                <p className="text-xs opacity-70 line-clamp-3">{t.shortDescription}</p>
              </Card>
            </Link>
          ))}
        </div>
      ...
      {/* CONSULTATION CTA */}
      {settings?.showConsultationSection && (
        <section className="px-4 py-16 text-center" style={{ background: "var(--sidebar)", color: "var(--sidebar-foreground)" }}>
          <div className="max-w-2xl mx-auto">
            ...
            <p className="opacity-80 mb-2">{settings?.consultationDescription}</p>
            {settings?.consultationPrice && (
              <p className="text-2xl font-bold mb-6" style={{ color: "var(--accent)" }}>
